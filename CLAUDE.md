...
- Network-layer invariants (all in `createSession`, `src/util.ts`, each with a test): response bodies are capped at 16 MB after decompression; a redirect leaving the scraper's `siteDomain` is refused with an error naming the destination; `caCert` is injected into the TLS hop tunneled through http(s) and SOCKS proxies (the proxy agents only apply it to the proxy hop); non-transient failures (body over cap, refused redirect, cancelled) are never retried. Untrusted HTML goes through `loadHtml` (full pages) or the converters, which refuse or text-strip markup nested deeper than 256 levels (counted as the HTML parser would build the tree) or longer than 2 million characters before any tree parser sees it; the `html` format's fallback is HTML-escaped text. Every option has a bound (`src/options.ts`), an explicit `null` is rejected, and validation errors are always `InvalidInputError`.
- Stress/fuzz harness: `../ts-jobspy-stress` (outside the repo) runs the built package as a consumer through an intercepting proxy (`mitm.mjs`, uses the public `proxies` + `caCert` options) with replayed real responses, transport abuse (`transport.mjs`), mutation fuzzing (`mutate.mjs`), option fuzzing (`limits.mjs`), load (`load.mjs`), slow origins (`slow.mjs`) and live limits (`live.mjs`). Re-run it after touching the network layer or a parser.
